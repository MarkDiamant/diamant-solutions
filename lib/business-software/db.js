const SB="https://iepqggrfenfqrqyzqyed.supabase.co";
...
const PUBLIC_SB="https://iepqggrfenfqrqyzqyed.supabase.co",PUBLIC_PK="sb_publishable_ivTCLFGFroexc-3IHe25bg_qhzg_GIy";
...
export async function tenantRecord(slug,host=null){
  const key=PUBLIC_PK;
  const r=await fetch(`${PUBLIC_SB}/rest/v1/rpc/business_software_public_tenant`,{method:"POST",headers:headers(key),body:JSON.stringify({p_slug:slug||null,p_host:host||null}),cache:"no-store"});
...
export async function publicTenantSettings(slug){
  const key=PUBLIC_PK;
  const r=await fetch(`${PUBLIC_SB}/rest/v1/rpc/business_software_public_settings`,{method:"POST",headers:headers(key),body:JSON.stringify({p_slug:slug}),cache:"no-store"});
  if(r.ok){const row=(await r.json())?.[0];if(row)return row;}
  if(SK){const tenant=await tenantRecord(slug,null);if(tenant?.id){const direct=await centralRest(`business_software_tenant_settings?tenant_id=eq.${encodeURIComponent(tenant.id)}&select=business_name,logo_url,accent_colour,app_config&limit=1`);if(direct.ok){const row=(await direct.json())?.[0];if(row)return row;}}}
  return null;
}
