...
export async function centralAuth(path,init={}){if(!SB||!SK)throw new Error("Central authentication service is not configured");return fetch(`${SB}/auth/v1/${path}`,{...init,headers:{apikey:SK,Authorization:`Bearer ${SK}`,"Content-Type":"application/json",...(init.headers||{})},cache:"no-store"});}
