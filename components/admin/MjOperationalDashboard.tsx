...
export default function MjOperationalDashboard(){
 ...
 <nav className="mt-6 flex flex-wrap gap-2">{["jobs","customers","quotes","invoices","payments","files","team"].map(v=><Link key={v} className="rounded-lg bg-[#17385f] px-4 py-3 text-sm font-bold capitalize text-white" href={"/bms-runtime/"+v}>{v}</Link>)}<Link className="rounded-lg border bg-white px-4 py-3 text-sm font-bold" href="/bms-runtime/jobs/new">+ New job</Link></nav>
