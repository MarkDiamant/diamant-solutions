...
export default function MjOperationalDirectory({view}:{view:View}){
 ...
 return <main className="min-h-screen bg-[#f5f5f2] px-4 py-7 text-[#141414] sm:px-5 lg:px-8"><div className="mx-auto max-w-[1500px]"><header className="flex flex-wrap justify-between gap-4"><div><h1 className="text-3xl font-black capitalize">{view}</h1><p className="text-sm text-black/50">M&J · Shared business management system · Isolated staging</p></div><div className="flex gap-2">{view==="jobs"&&<Link className="rounded-xl bg-[#e66a24] px-4 py-2 text-white" href="/bms-runtime/jobs/new">+ New job</Link>}{view==="customers"&&<button className="rounded-xl bg-[#e66a24] px-4 py-2 text-white" onClick={addCustomer}>+ New customer</button>}<Link className="rounded-xl border bg-white px-4 py-2" href="/bms-mj-preview">Staging account</Link></div></header>
 <nav className="my-5 flex flex-wrap gap-2">{(Object.keys(resources) as View[]).map(v=><Link key={v} className={"rounded-xl px-3 py-2 text-sm font-bold "+(v===view?"bg-[#e66a24] text-white":"bg-white")} href={"/bms-runtime/"+(v==="team"?"team":v)}>{v}</Link>)}</nav>
