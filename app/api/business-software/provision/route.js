...
export async function POST(request){
 try{
  ...
  const created=await Promise.all([
   centralRest("business_software_tenant_settings",{method:"POST",headers:{Prefer:"return=minimal"},body:JSON.stringify({tenant_id:tenant.id,business_name:businessName,email:order.customer_email||null,timezone:"Europe/London",currency:"GBP",quote_prefix:String(body.quotePrefix||"Q").slice(0,8),invoice_prefix:String(body.invoicePrefix||"INV").slice(0,8),features:{quotes:true,invoices:true,gmail:true,xero:true},app_config:{businessName,systemName:"Business Management Software",logoUrl:"/favicon.svg",accentColour:"#e66a24",workforceTitle:"Team",workforceSingular:"team member",workforceRoles:["Engineer","Technician","Contractor","Subcontractor","Surveyor","Project manager","Other"],managers:[{value:"M1",label:String(body.adminName||"Owner").trim()||"Owner"}],workTypes:["Installation","Repair","Maintenance","Service","Survey","Consultation","Project","Other"],finishOptions:["Standard","Other"],enquirySources:["WhatsApp","Email","Website","Phone","Referral","Existing Customer","Other"],fileCategories:["Site Survey","Before","Drawing","Work","After","Other"],quoteTemplate:"clean",invoiceTemplate:"clean",tenantKey:slug,plan:{includedUsers:1,licensedUsers:Math.max(1,Number(order.users||1)),additionalUserMonthly:10,aiAssistantMonthly:15,annualMonthsCharged:10,aiIncluded:Boolean(order.ai_included)},ai:{enabled:Boolean(order.ai_included),textAssist:Boolean(order.ai_included),voiceAssist:Boolean(order.ai_included),includedTextActions:null,includedVoiceMinutes:null},billing:{mode:"paid",interval:order.billing_interval||"monthly",status:"active",resumeUrl:"",customerId:order.stripe_customer_id||"",subscriptionId:order.stripe_subscription_id||"",currentPeriodEnd:"",cancelAtPeriodEnd:false},modules:{photos:true,workforce:true,commission:true,payments:true,costs:true,finishes:true,siteVisits:true,scheduling:true,quotes:true,invoices:true}}})}),
   ...
   centralRest("business_software_job_sequences",{method:"POST",headers:{Prefer:"resolution=merge-duplicates,return=minimal"},body:JSON.stringify({tenant_id:tenant.id,last_number:0})}),
  ...
  if(created.some(r=>r&&r.ok===false)){
    await Promise.all([
      ...
      centralRest("business_software_job_sequences?tenant_id=eq."+tenant.id,{method:"DELETE",headers:{Prefer:"return=minimal"}})
    ]);
