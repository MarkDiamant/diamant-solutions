...
export async function GET(request){
  ...
  const upsert=await centralRest("business_software_integrations?on_conflict=tenant_id,provider",{method:"POST",headers:{Prefer:"resolution=merge-duplicates,return=minimal"},body:JSON.stringify({tenant_id:tx.tenant_id,provider:"google",provider_account:info.email||null,status:"connected",scopes:String(token.scope||"").split(" ").filter(Boolean),connected_at:new Date().toISOString(),disconnected_at:null,access_token_ciphertext:encrypt(token.access_token),refresh_token_ciphertext:encrypt(token.refresh_token),token_expires_at:expiresAt,metadata:{email_verified:Boolean(info.email_verified)}})});
