...
export async function GET(request){
 try{
  ...
  const org = connections.length === 1 ? connections[0] : null;
  ...
  if (!org) {
    const pending=await centralRest("business_software_integrations?on_conflict=tenant_id,provider",{method:"POST",headers:{Prefer:"resolution=merge-duplicates,return=minimal"},body:JSON.stringify({tenant_id:tx.tenant_id,provider:"xero",provider_account:null,status:"pending",external_tenant_id:null,scopes:String(token.scope||"").split(" ").filter(Boolean),connected_at:null,disconnected_at:null,access_token_ciphertext:encrypt(token.access_token),refresh_token_ciphertext:token.refresh_token?encrypt(token.refresh_token):oldRefresh,token_expires_at:expiresAt,metadata:{organisations:connections.map(c=>({tenantId:c.tenantId,tenantName:c.tenantName||null,tenantType:c.tenantType||null}))}})});
    if(!pending.ok)return NextResponse.redirect(destination(tx.return_origin,"error"));
    return NextResponse.redirect(destination(tx.return_origin,"choose_org"));
  }
