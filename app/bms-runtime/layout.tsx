...
export default async function BmsRuntimeLayout({children}:{children:React.ReactNode}){
 const tenant=await runtimeTenant(),mj=tenant?.slug==="mjmetal";
 if(mj)return <div className="bms-shell"><header className="!sticky !top-0 !z-40 !flex !flex-wrap !items-center !justify-between !gap-3 !border-b !border-black/10 !bg-white !px-4 !py-3 print:hidden"><Link href="/" className="text-xl font-black tracking-tight text-[#172b4d]">M&J <span className="text-[#e66a24]">Management</span></Link><nav className="!flex !flex-wrap !gap-2 !bg-white text-sm font-bold">{[["Dashboard","/"],["Jobs","/jobs"],["Customers","/customers"],["Quotes","/quotes"],["Invoices","/invoices"],["Payments","/payments"],["Files","/files"],["Team","/team"],["Integrations","/integrations"]].map(([name,url])=><Link key={url} href={url} className="rounded-lg px-2 py-2 text-[#172b4d] hover:bg-[#fff0e4] hover:text-[#e66a24]">{name}</Link>)}</nav></header>{children}<footer className="bms-footer print:hidden"><DiamantCredit dark/></footer></div>;
